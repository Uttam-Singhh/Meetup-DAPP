  ...
  async function newRSVP() {
    try {
      ...
      if (rsvpContract) {
        ...
        console.log("Minting transaction transaction:", txn.hash);

        await txn.wait();
        console.log("Transaction minted...", txn.hash);
        setSuccess(true);
        setLoading(false);

        setMessage(`You've successfully rsvped for ${event.name}`);
      } else {
        console.log("Error getting contract.");
      }
    } catch (err) {
      setSuccess(false);
      setMessage("Something went wrong with your transaction...");
      setLoading(false);
